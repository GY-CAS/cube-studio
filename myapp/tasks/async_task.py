...
def update_dataset(task,dataset_id):
    ...
    with session_scope(nullpool=True) as dbsession:
        """ 
        使用 session_scope 上下文管理器来获取数据库会话。
        nullpool=True 表示使用一个空的连接池，这可能意味着不使用任何特定的连接池，直接创建和关闭数据库连接。
        """
        try:
            dataset = dbsession.query(Dataset).filter_by(id=dataset_id).first()# 从数据库中查询指定id的数据集
            ...
            if dataset.path:
                """ 
                如果数据集有一个本地路径,将路径分割，然后遍历，
                """
                paths = dataset.path.split("\n")
                for path in paths:
                    file_name = path[path.rindex("/") + 1:]  #从路径中提取文件名
                    local_path = os.path.join('/home/myapp/myapp/static/', path.lstrip('/')) #构建本地文件完整路径
                    if os.path.exists(local_path): #查看本地文件是否存在
                        # 对文件直接复制
            ...
            elif dataset.download_url: #如果数据集没有本地路径，但是有下载链接
                download_urls = dataset.download_url.split("\n")
                for download_url in download_urls:
                    try:
                        import requests
                        filename = download_url.split("/")[-1]  #遍历每个下载链接
                        try_num=0
                        while try_num<3: #最多循环3次下载文件
                            try_num+=1
                            response = requests.get(download_url)
                            with open(remote_dir + '/' + filename, 'wb') as f:  #尝试获取下载链接的文件内容。
