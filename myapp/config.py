##################################
# copy from install\docker\config.py
#################################
...
from dateutil import tz  #从 dateutil 模块导入 tz，用于处理时区相关的功能。
...
"""
SQLALCHEMY_POOL_SIZE = 300: 这行代码设置了 SQLAlchemy 数据库连接池的大小。SQLAlchemy 是一个 ORM（对象关系映射器），它管理数据库连接。连接池大小指的是它将维护的连接数量。在这个例子中，SQLAlchemy 将维护最多 300 个数据库连接。
SQLALCHEMY_POOL_RECYCLE = 300: 这行代码设置了连接的回收周期，即 SQLAlchemy 将回收连接的时间间隔。在这个例子中，连接会在每个 300 秒后被回收。需要注意的是，这个值应该小于数据库的超时时间，否则可能会导致连接丢失。
SQLALCHEMY_MAX_OVERFLOW = 800: 这行代码设置了最大溢出连接数。当所有连接都在使用中时，额外的连接数量可以超过连接池的大小。在这个例子中，SQLAlchemy 可以管理的最大连接数是 800。
SQLALCHEMY_TRACK_MODIFICATIONS=False: 这行代码设置了一个标志，指示 SQLAlchemy 是否跟踪数据库对象的变化。如果设置为 False，SQLAlchemy 将不会追踪对象的修改，这可能会提高性能。
这些设置是 SQLAlchemy 连接管理的关键部分，它们控制着数据库连接的创建、维护和重用。通过适当配置，可以优化应用程序与数据库的交互，提高性能和稳定性。
...
username = 'root'
password = 'bc7b7ac2'
host = '172.16.1.20:12020'
dbname = 'kubeflow'
# 构建MySQL连接URL
url = f'mysql+pymysql://{username}:{password}@{host}/{dbname}?charset=utf8'
SQLALCHEMY_DATABASE_URI = os.getenv('MYSQL_SERVICE', url)
#================================
