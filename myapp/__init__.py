...
APP_DIR = os.path.dirname(__file__)  #__file__ 表示当前文本位置
CONFIG_MODULE = os.environ.get("MYAPP_CONFIG", "myapp.config")  
#从环境变量中获取名为 MYAPP_CONFIG 的设置，如果未设置则使用默认值 "myapp.config"。这个设置用于指定 Flask 应用程序的配置模块。

# app = Flask(__name__,static_url_path='/static',static_folder='static',template_folder='templates')
app = Flask(__name__)  # ,static_folder='/mnt',static_url_path='/mnt'

app.config.from_object(CONFIG_MODULE) #从环境变量中读取更新app的配置
conf = app.config

if conf.get('DATA_DIR', ''):  #如果设置了DATA_DIR，则创建该目录
...
def parse_manifest_json():
    ...
    try:
        with open(MANIFEST_FILE, "r") as f:
            ...
            manifest = full_manifest.get("entrypoints", {})  #读取entrypoints字段存储到全局变量manifest中
    except Exception:
        pass
...
"""
获取与给定filename相关的JavaScript文件的路径列表。
如果应用程序处于调试模式，则重新解析manifest文件。
"""
...
def check_login():
    """  
    这是一个请求前处理函数，用于检查用户是否已登录。
    如果用户未登录，则根据请求中的cookie或Authorization头进行身份验证。
    """
...
def myapp_after_request(resp):
    """这是一个请求后处理函数，用于在响应中设置cookie，如用户名和会话ID"""
    try:
...
def apply_http_headers(response):
    """Applies the configuration's http headers to all responses
        这是一个请求后处理函数，用于将配置中的HTTP头添加到所有响应中。
    """
