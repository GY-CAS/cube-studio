import pysnooper
from flask_appbuilder import Model  #用于定义数据库表的基类
...
from sqlalchemy import Column, Integer, String  #用于定义数据库表-列--的类
...
class Dataset(Model,AuditMixinNullable,MyappModelBase):
    __tablename__ = 'dataset'  #用于指定与这个类对应的数据库表名

    #类中定义了多个字段，如 id、name、label 等，这些字段对应于数据库表中的列。每个字段都有其数据类型、是否可为空和其他属性。
    ...
    source_type = Column(String(200), nullable=True,comment='数据集来源，开源，资产，采购')  #来源类型
...
<a target=_blank href='{img_url}'>
  <img style='height:50px; width:50px; border-radius:10%' src='{img_url}'>
    ...
    @property
    def download_url_html(self):  #下载地址
        """
        该函数的作用是将实例变量download_url中的URL列表转换为HTML代码。它通过将URL列表中的每个URL转换为HTML的<a>标签，
        并在每个标签后添加<br>标签来实现。最后，将生成的HTML代码封装在一个<div>标签中，并返回作为Markup对象的结果。
        """
    ...
    def card(self):
        """
        card 属性返回一个 HTML 内容，表示数据集的卡片视图，其中包含数据集的图标、描述、操作按钮等。
        两个操作按钮，一个用于数据集的标注，另一个用于数据集的计算。这两个按钮的链接分别指向标注和计算的 API。
        """
