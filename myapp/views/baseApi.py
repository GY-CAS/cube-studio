import copy
import csv
import functools #这个模块包含了一些用于函数式编程的辅助函数，如 update_wrapper 和 wraps。这些函数可以用来装饰器中包装函数。
import json
import logging
import re
import traceback #这个模块提供了跟踪和格式化 Python 回溯信息的功能。在出现异常时，这可能被用来获取和显示错误堆栈信息。
import urllib.parse
import os
from flask import Markup, Response, current_app, make_response, send_file, flash, g, jsonify, request
"""
Markup: Markup 是一个类，用于安全地处理可能包含HTML标签的字符串。它可以确保字符串在模板渲染时不会意外转义，而是作为原始HTML输出。

Response: Response 是Flask中的一个类，用于创建HTTP响应对象。你可以用它来返回自定义的内容类型、状态码以及其他HTTP头部信息。

current_app: current_app 是一个代理，它引用当前激活的应用实例。这意味着无论你在哪个应用上下文中调用它，它都会返回正确的应用对象，这对于跨应用共享资源或者获取全局配置非常有用。

make_response: make_response 是一个工厂函数，用来构建HTTP响应。通常接收一个字符串或一个字节串作为主体内容，并可以根据需要添加HTTP头信息，然后返回一个Response对象。

send_file: send_file 函数用于发送静态文件，例如图片、文档等。它可以高效且安全地发送文件到客户端，并能处理诸如缓存控制、断点续传等高级特性。

flash: flash 函数用于在用户会话中存储临时消息，通常用于在请求之间传递状态信息，比如成功或错误提示信息，在下一个请求的模板中可以通过 flashed messages 得到并展示给用户。

g: g 是一个全局变量对象，它在每个请求开始时被重置，并在整个请求生命周期内可用。它为每个请求提供了临时存储请求级数据的地方。
"""
from inspect import isfunction

from sqlalchemy import create_engine
"""
这行代码是从 SQLAlchemy 库中导入 create_engine 函数。SQLAlchemy 是 Python 中的一个 SQL 工具包与对象关系映射 (ORM) 框架，
它提供了灵活的方式来操作关系型数据库。

create_engine 函数是 SQLAlchemy 中的核心部分之一，它的主要作用是根据给定的数据库连接字符串创建一个数据库引擎。
这个引擎代表了数据库连接池以及数据库DDL/DML操作的来源。当你调用 create_engine 并传入适当的数据库连接字符串时，
它会初始化与特定数据库系统的连接，允许你进一步进行数据的操作，如读取、写入及管理数据库。
"""
from flask_appbuilder.actions import action
""" 
action 函数在 Flask-AppBuilder 中主要用于定义模型视图上的自定义操作。当您在模型视图上定义一个 action 后，它通常会出现在视图的相关操作列表中，用户可以在前端界面触发该操作。
这个操作可以是你自定义的任意复杂逻辑，比如批量更新数据、执行某种业务流程等。
"""
from flask_babel import gettext as __
from flask_babel import lazy_gettext as _
from flask_appbuilder.actions import ActionItem
""" 
ActionItem 是一个用于在视图中定义定制动作的类。通过创建 ActionItem 实例，您可以向视图添加额外的功能按钮或者菜单项，
这些动作可以是预定义的操作（如编辑、删除等），也可以是自定义的业务逻辑。
"""
from flask.globals import session
import jsonschema  #这个导入提供了 JSON Schema 验证功能。在处理 API 请求时，这可能被用来验证 JSON 数据的有效性。
from marshmallow import ValidationError #这个导入提供了 Marshmallow 验证错误处理。在处理 API 请求时，这可能被用来处理数据验证错误。


from marshmallow_sqlalchemy.fields import Related, RelatedList 
#是一个用于将SQLAlchemy模型转换为JSON的库，
#其中 Related 和 RelatedList 分别用于处理一对一或多对一以及一对多或多对多关系字段的序列化和反序列化。

import prison #是一个用于净化和规范化文本以防止XSS攻击的库，常用于将用户输入转化为安全的HTML。

####数据库操作相关
from sqlalchemy.exc import IntegrityError #IntegrityError 是SQLAlchemy抛出的一个异常，当数据库约束（如唯一性、外键等）违反时会发生。
from sqlalchemy.orm.attributes import InstrumentedAttribute #是SQLAlchemy ORM中用于表示对象属性的类，这些属性实际上是与数据库列相对应的。
from sqlalchemy.orm.properties import ColumnProperty  #是SQLAlchemy ORM中的一个属性描述符，它代表映射到数据库表列的类属性。
from sqlalchemy.orm.relationships import RelationshipProperty #也是SQLAlchemy ORM中的属性描述符，但它代表的是类之间的关系（如一对多、多对多或一对一）。
from sqlalchemy.sql import sqltypes

from werkzeug.exceptions import BadRequest #BadRequest 是Werkzeug库中的HTTP异常，当接收到无效的HTTP请求时，通常会抛出此异常。
from marshmallow import validate #validate 是Marshmallow库中的一个模块，包含了一系列内置验证器，用于在序列化或反序列化过程中对字段值进行校验。

from wtforms import validators 
#wtforms 是一个Python库，用于处理web表单和验证。这里的 validators 模块包含了各种表单字段验证器，
#如数据有效性检查、必填字段检查等。在Web应用中，通常配合表单字段一起使用，确保用户提交的数据符合预期。

from flask_appbuilder._compat import as_unicode  # 这个导入提供了兼容性函数，用于确保在 Python 2 和 Python 3 之间转换字符串。

"""
这段代码从 Flask-AppBuilder 库中导入了一系列常量，这些常量主要用于在 Flask-AppBuilder 构建的 REST API 接口中定义响应和请求的键值（Key)
这些常量在构建 Flask-AppBuilder 的 API 规范和处理客户端请求时起到关键作用，使得前后端交互更为规范且易于维护。
"""
...
from flask import (
    abort, #abort 函数用于立即停止处理请求，并返回指定的 HTTP 错误状态码给客户端
)

from flask_appbuilder.exceptions import FABException, InvalidOrderByColumnFABException
"""
FABException: Flask-AppBuilder 自定义的基础异常类，通常用于捕获和抛出自定义的框架内部异常。
InvalidOrderByColumnFABException: 这是一个具体的自定义异常，当尝试按无效的列排序时抛出
"""
from flask_appbuilder.security.decorators import permission_name, protect, has_access
"""
permission_name: 通常用于获取权限名称的装饰器，以便在视图函数上标记所需的权限。
protect: 装饰器，用于保护视图函数，只有拥有相应权限的用户才能访问。
has_access: 函数，用于检查当前用户是否具有访问特定资源的权限。
"""
from flask_appbuilder.api import BaseModelApi, BaseApi, ModelRestApi
""" 
BaseModelApi: Flask-AppBuilder 提供的基础API类，用于构建基于SQLAlchemy模型的RESTful API。
BaseApi: Flask-AppBuilder 的基础API类，提供了一些通用的API处理功能。
ModelRestApi: Flask-AppBuilder 中针对SQLAlchemy模型设计的RESTful API扩展基类，用于简化基于模型的API接口开发，包括CRUD操作等功能。
"""
from myapp import app, appbuilder, db, event_logger, cache   #db:数据库,
...
class MyappModelRestApi(ModelRestApi):
    ...
    check_edit_permission = None  #  很多定义在View_XX 类文件中，如赋值一个函数
    ...
    edit_form_extra_fields = {} #修改数据集需要填写
    add_form_extra_fields = {}  #新增数据集需要填写字段
    add_fieldsets = []
    edit_fieldsets = []
    show_fieldsets = []
    ...
    def response(code, **kwargs):
        ...
        """
        # 添flash的信息,获取 Flash 消息,使用 session.get 方法从会话中获取 Flash 消息列表。
        #Flash 消息通常用于临时存储一些需要在前端显示的信息，例如错误消息或提示
        flashes = session.get("_flashes", [])  #闪现消息Flask-flash
    ...
    """ 
    这些 merge 装饰器的作用是为 api_info 函数的响应添加额外的数据。它们允许在响应中合并来自不同函数的数据，
    这些数据是根据请求参数中的特定键来选择的。每个装饰器都指定了要合并的数据的来源函数和用于选择该数据的 RISON 键
    """

    @expose("/_info", methods=["GET"])
    @merge_response_func(merge_more_info, 'more_info')#这个装饰器告诉 api_info 函数，当请求参数中包含 'more_info' 键时，应该调用 merge_more_info 函数并将它的返回值合并到响应中。
    ...
    def api_info(self, **kwargs):
        _response = dict()
        _args = kwargs.get("rison", {}) #从请求参数中获取RISON参数。
        _args.update(request.args)
        id = _args.get(self.primary_key, '') #获取请求参数中的主键值
        if id:
            ...
            if self.check_edit_permission:  #检查编辑权限
        ...
        """  
        常用于将一组键值对映射到响应对象中。这个方法通常在API视图函数中使用，以便在响应中包含自定义的数据和元数据
        此处用来构建一个包含API元数据的响应。这通常用于提供有关API端点、请求处理、数据模型等信息。
            _response: 这是一个初始化的响应对象，它将包含最终要返回的数据。
            self.api_info: 这可能是一个包含API元数据的函数或方法。
            _args: 这可能是一个包含API请求参数的字典。
            **_args: 这允许您传递额外的关键字参数到self.api_info函数。
        """
    ...
    def show_expand(self,json_data_src):
        json_data = copy.deepcopy(json_data_src) #深拷贝
        for column_name in self.expand_columns: #遍历扩展字段，数据集部分为空
            ...
            for col in new_columns:
                json_data[col] = expand_value.get(col,None)#添加到json_data中
    ...
    def api_show(self, pk, **kwargs):
        ...
        item = self.datamodel.get(pk, self._base_filters) #根据id获取相应数据集，并进行基础权限过滤
        if not item:
            return self.response_error(404, "Not found")

        if self.pre_show: #数据集部分pass
            self.pre_show(item)

        _response = dict()
        _args = kwargs.get("rison", {}) #从请求参数中提取数据，并更新_args字典。
        ...
        #从_args中获取选择列，并过滤掉不在self.show_columns中的列
        ...
        if self.expand_columns:
            _response['data'] = self.show_expand(_response['data'])  # 添加扩展字段展开显示
        back = self.pre_show_res(_response)#数据集部分为pass
        back_data = {
    ...
    def api_list(self, **kwargs):
        ...
        try:
            req_json = request.get_json(silent=True) or {}  #获取JSON请求体，如果失败则默认为空字典。理解是输入表单
        except Exception as e:
            print(e)
            req_json = {}

        _args = req_json or {}
        _args.update(request.args)  #更新请求参数
        # 应对那些get无法传递body的请求，也可以把body放在url里面
        if 'form_data' in request.args:
            _args.update(json.loads(request.args.get('form_data')))

        for key in request.args:  #遍历所有GET参数
            if key!='form_data':
                _args[key.replace('form_data','').replace('[','').replace(']','')]=request.args[key] #处理’form_data’为前缀的键，去掉前缀并更新到_args中

        if self.pre_list_req:  #如果存在预处理函数，则调用它
            _args = self.pre_list_req(_args)

        # handle select columns
        select_cols = _args.get(API_SELECT_COLUMNS_RIS_KEY, []) #获取选中的列
        _pruned_select_cols = [col for col in select_cols if col in list(set(self.list_columns + self.show_columns))] #过滤掉无效的列
        self.set_response_key_mappings(
            _response,
            self.get_list,
            _args,
        ...
        if _pruned_select_cols: #如果存在选中的列，继续处理
            _list_model_schema = self.model2schemaconverter.convert(_pruned_select_cols) #转换模型到schema
        else:
            _list_model_schema = self.list_model_schema #否则使用默认列
        # handle filters
        try:
            # 参数缩写都在每个filter的arg_name
            joined_filters = self._handle_filters_args(_args) #将过滤请求参数解析为数据库格式的查询对象
        except FABException as e:
            return self.response_error(400, message=str(e))
        # handle base order
        try:
            order_column, order_direction = self._handle_order_args(_args)#转换为排序的数据库格式查询参数
        except InvalidOrderByColumnFABException as e:
            return self.response_error(400, message=str(e))
        # handle pagination
        page_index, page_size = self._handle_page_args(_args) #处理如何从请求参数中提取分页信息，并将其转换为可以用于数据库查询的格式
        # Make the query，构建查询列
        query_select_columns = _pruned_select_cols or self.list_columns

        # @pysnooper.snoop()
        def get_favorite(): #辅助函数，获取用户收藏
        ...
        if _args.get('only_favorite', False):
            ...
            joined_filters = self._filters.get_joined_filters(self._base_filters)  # 将基础filter加入到过滤器中，权限过滤

        #执行查询，获取符合条件的数据列表和总记录数
        ...
        if self.post_list:  #如果存在后处理函数，则调用它
        ...
        data = _list_model_schema.dump(lst, many=True)  #用定义的schema将查询结果转换为字典列表。
    ...
    def api_add(self):
        ...
        try:
            ...
            json_data = {key: json_data[key] for key in json_data if key in self.add_columns} #过滤JSON数据，只保留添加列
            item = self.add_model_schema.load(json_data) #使用定义的schema将JSON数据转换为模型实例。
            # item = self.add_model_schema.load(data)
        except Exception as err:
            flash(str(err),'error')
            return self.response_error(422, message=str(err))
        # This validates custom Schema with custom validations
        if isinstance(item, dict):#返回422，数据无效
            return self.response_error(422, message=item.errors)
        try:
            self.pre_add(item) #检查数据集拥有者，见View_dataset.py
            self.datamodel.add(item, raise_exception=True) #添加到数据集库表Dataset
            self.post_add(item)# 数据集相关操作为pass
    ...
    def _deserialize_pk_if_composite(self, pk):
        """  
        如果主键是一个复合键（即由多个字段组成），这个函数将尝试反序列化主键。
        它使用date_deserializer函数来处理日期格式的反序列化。
        """
    ...
    def _handle_page_args(self, rison_args):
        """
            ...
            FAB_API_MAX_PAGE_SIZE，提取分页信息，进行处理和验证

        :param rison_args:
        :return: (tuple) page, page_size
        """
        page = rison_args.get(API_PAGE_INDEX_RIS_KEY, 0) #从rison_args中获取页码，如果没有提供，默认为0
        page_size = rison_args.get(API_PAGE_SIZE_RIS_KEY, self.page_size) #从rison_args中获取每页的记录数，如果没有提供，默认为self.page_size。
        return self._sanitize_page_args(page, page_size)

    # @pysnooper.snoop()
    def _sanitize_page_args(self, page, page_size):
        """  
        用于处理分页参数，如page和page_size。
        它会设置默认值，并确保分页大小不超过FAB_API_MAX_PAGE_SIZE。
        """
        _page = page or 0
    ...
    def _handle_order_args(self, rison_args):
        """
            Help function to handle rison order arguments
            
            用于处理API请求中的排序参数。这个函数的主要目的是将来自请求的排序参数解析并转换为一个可以用于数据库查询的对象。
            
        :param rison_args:
        :return:
        """
        order_column = rison_args.get(API_ORDER_COLUMN_RIS_KEY, "")  #获取排序列，默认为空
        order_direction = rison_args.get(API_ORDER_DIRECTION_RIS_KEY, "") #获取排序方向，默认为空
        if not order_column and self.base_order: #如果没有指定排序且存在基础排序，则返回基础排序
            return self.base_order
        if not order_column:
            return "", ""
        elif order_column not in self.order_columns: #如果排序列不在允许的排序列中，则抛出异常
    ...
    def _handle_filters_args(self, rison_args):  
        """ 
        函数用于处理API请求中的过滤参数。其目的是将来自请求的过滤参数解析并转换为一个可以用于数据库查询的对象
        """
        self._filters.clear_filters() #清空过滤器
        self._filters.rest_add_filters(rison_args.get(API_FILTERS_RIS_KEY, []))#添加过滤器
        return self._filters.get_joined_filters(self._base_filters) #添加新增过滤器并返回
    ...
    def _description_columns_json(self, cols=None):
        """
            ...
            用于准备一个包含列标签的JSON可序列化字典。
            它将列标签转换为Unicode字符串，以确保它们可以在JSON响应中正确显示。
        """
        ret = {}
        cols = cols or []
    ...
    def _merge_update_item(self, model_item, data):
        ...
        用于合并模型项和数据结构。
        它将数据库中的模型与来自请求的数据合并，以支持PATCH操作。
        """
