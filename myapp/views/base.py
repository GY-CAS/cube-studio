...
class MyappFilter(BaseFilter):
    ...
    函数原本可能存在于 SecurityManager 类中，但是在每次检查时都会进行一次数据库的往返查询（即数据库查询操作）。
    这意味着如果 SecurityManager 中的函数每次都被调用，那么每次都需要查询数据库，这在性能上可能是不高效的。
    通过缓存角色对象（role objects）来实现多次检查，而只查询数据库一次。这样做的好处是，一旦数据库查询完成并返回了角色对象，
    后续的多次检查就可以直接使用缓存中的对象，而不需要再次查询数据库。这大大减少了数据库的访问次数，从而提高了效率。
    """

    def get_user_roles(self):  #返回当前用户角色
        return get_user_roles()

    def get_all_permissions(self): # 
        """Returns a set of tuples with the perm name and view menu name，
        返回一个包含所有用户角色的权限名称和视图菜单名称的集合。
        它遍历用户的所有角色，并为每个角色查询其权限，将这些权限以元组的形式（权限名称， 视图菜单名称）添加到集合中。
        """
        perms = set()
    ...
    def has_role(self, role_name_or_list):
        ...
        '''  这个方法检查用户是否具有指定的角色名称。如果 role_name_or_list 不是一个列表，它会将其转换为列表。然后，它检查用户的所有角色中是否至少有一个角色的名称在列表中。'''
        if not isinstance(role_name_or_list, list):
            role_name_or_list = [role_name_or_list]
        return any([r.name in role_name_or_list for r in self.get_user_roles()])

    def has_perm(self, permission_name, view_menu_name): #查看用户是否拥有指定的权限
